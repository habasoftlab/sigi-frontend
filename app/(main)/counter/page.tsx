...
interface Product {
id: string; // PRD-001A
name: string; // Tarjetas básicas (couché 300g, 1 cara)
pricePerUnit: number | null; // 1.80
pricePerPackage: number | null; // 180
packageSize: number | null; // 100
productionTime: string; // 2 días
format: string; // 9x5 cm
saleUnit: string; // Paquete (100)
minOrder: number; // 100
volumeDiscount: number; // 1000
}
...
// Por ahora, lo asignado directamente a la cotización
...
return (
    <Button
        ...
        className="w-6 h-4 flex items-center justify-center hover:scale-110 transition-transform duration-150"
    />
);
};
